...
# Source stems, without extension. Photos get re-exported in whatever format the
# phone or editor produced, so the extension is resolved at run time instead of
# being pinned here — swapping a jpeg for a png should not break the build.
FILES = {
    "preran": "Preran_PFP",
    "shamanth": "Shamanth_PFP",
    "shreya": "Shreya_PFP",
    "shashank": "Shashank_PFP",
    "chithsukhi": "Chitsukhi_PFP",
}
...
def resolve_source(stem: str) -> Path:
    """First existing Team_PFP file matching the stem, in EXTENSIONS order."""
    for ext in EXTENSIONS:
        candidate = SRC / f"{stem}{ext}"
        if candidate.exists():
            return candidate
    tried = ", ".join(f"{stem}{ext}" for ext in EXTENSIONS)
    raise FileNotFoundError(f"missing source portrait in {SRC}: tried {tried}")
